...
load('400_global_results.mat')
load('400_damper_results.mat')
load('400_convection_results.mat')
load('400_EG_results.mat')
...
% 
rank = 10; %latent dimension

% y1 = conv_fluid_T;
% y2 = conv_fluid_Q_flow;
% y3 = Q_flow;
% y5 = conv_solid_Q_flow;
% y6 = conv_solid_T;
% y7 = conv_dT;

y1 = damper_flange_a_f;
y2 = damper_flange_a_s;
y3 = damper_flange_b_f;
y5 = damper_heatPort_Q_flow;
y6 = damper_heatPort_T;
y7 = damper_lossPower;
% y1_vec = reshape(y1,[],1);
% y2_vec = reshape(y2,[],1);
% y3_vec = reshape(y3,[],1);
...
TAmbient = TAmbient ./ max(TAmbient);
sine1_start = sine1_start ./ max(sine1_start);
sine1_amp = sine1_amp ./ max(sine1_amp);
sine1_freqHz = sine1_freqHz ./ max(sine1_freqHz);
sine1_phase = sine1_phase ./ max(sine1_phase);
sine1_offset = sine1_offset ./ max(sine1_offset);
damper_d = damper_d ./ max(damper_d);
k = k ./ max(k);
y4 = [TAmbient', k', sine1_start', sine1_amp', sine1_freqHz', sine1_phase', sine1_offset'];
