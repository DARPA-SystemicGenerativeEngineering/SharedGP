function model = scigplvm_infere_v3(model,k,yi_star)
...
%     fastDerivativeCheck(@(params) log_evidence_share_more(params,kerType, a0, b0, rank, yTr, N_all,model), params)
    fastDerivativeCheck(@(params)  log_evidence_share_miss(params,params_length, a0, b0, yTr,rank,N_total), params)
...
%     nIte = 100;
    ...
    new_params = minFunc(@(params) log_evidence_share_miss(params,params_length, a0, b0, yTr,rank,N_total), params, opt);
    params = new_params;
...
%%
    ...
    for i = 1:length(yTr)
        ...
        Knn2{i} = ker_cross(u_invGP, U, ker_params{i}); 
        y_star_invGP{i} = Knn2{i}*(Sigma{i}\yTr{i}(1:N,:));     %prediction based on simple invGP.
    end
...
%     model.train_pred = y_pred;
    ...
    model.y_star_invGP = y_star_invGP;
    model.u_star_invGP = u_invGP;
